--Objeto del Jugador
PlayerObj = objPhys:new()

--Constructor
function PlayerObj:new(_table,_x,_y,_boxw,_boxh)
  _table = _table or objPhys:new(_table,_x,_y,_boxw,_boxh)
  setmetatable(_table,self)
  self.__index = self
  
  self.Mask = {0,0,0,0}
  self.hsp = 0 ; self.vsp = 0
  self.maxspeed = 1 ; self.movspeed = 1 ; self.Desmov = 1
  return _table
end
...
function PlayerObj:moving(_dt)
  ...
  if self.vsp == math.minmax(-self.maxspeed,self.maxspeed, self.vsp) and KVP ~= 0 then
    self.vsp = self.vsp + KVP * self.movspeed
  ...
  self.subx = self.subx + self.hsp *_dt ; self.suby = self.suby + self.vsp *_dt
